...
from app.modules.platform.auth.session.schemas import DeviceInfo, SessionInfo
...
class SessionService:
    ...
    async def list_sessions(
        ...
        """List all active (non-revoked, non-expired, verified) sessions for a user."""
        now = datetime.now(timezone.utc)
        result = await self.db.execute(
            select(RefreshToken).where(
                RefreshToken.user_id == user_id,
                RefreshToken.revoked.is_(False),
                RefreshToken.verified.is_(True),
        ...
        sessions = []
        for t in tokens:
            parsed_ua = parse_user_agent(t.user_agent)
            sessions.append(
                SessionInfo(
                    id=t.id,
                    ...
                    device_info=DeviceInfo(**parsed_ua),
                )
            )
        return sessions
